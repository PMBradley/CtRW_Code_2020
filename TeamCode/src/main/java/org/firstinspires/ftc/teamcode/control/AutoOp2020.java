...
public class AutoOp2020 extends LinearOpMode {
    ...
    public static TargetDrivePosition shootPos        = new TargetDrivePosition(-51.1, -8, Math.toRadians(4.2));
    ...
    ArrayList<AutoTask> setupAutoTasks(){
        ...
        autoTasks.add(new AutoTask("Shoot Rings", 2, new TargetDrivePosition(shootPos.getX(), shootPos.getY(), shootPos.getHeading()), atLocationTasks).setCompleted(true)); // set completed so that the pathing algorithm won't consider it until it is set true (which will happen once its prerequisite task becomes completed)
